...
func TestDMuxN(t *testing.T) {
	dmux4, err := hw.Chip("myDMux4", "in[4], sel", "a[4], b[4]",
...
func TestMuxMWayN(t *testing.T) {
	mux4 := hl.MuxN(4)
	mux44, err := hw.Chip("myMux4Way4", "a[4], b[4], c[4], d[4], sel[2]", "out[4]",
...
func TestDMuxNWay(t *testing.T) {
	dmux4, err := hw.Chip("myDMux4Way", "in, sel[2]", "a, b, c, d",
...
func TestDMuxMWayN(t *testing.T) {
	dmux4 := hl.DMuxN(4)
	dmux44, err := hw.Chip("myDMux4Way4", "in[4], sel[2]", "a[4], b[4], c[4], d[4]",
