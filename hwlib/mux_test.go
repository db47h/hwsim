...
func TestMuxN(t *testing.T) {
	m, err := hw.Chip("myMux16", "a[4], b[4], sel", "out[4]", hw.Parts{
...
func TestDMuxN(t *testing.T) {
	dmux4, err := hw.Chip("myDMux4", "in[4], sel", "a[4], b[4]", hw.Parts{
...
func TestMuxMWayN(t *testing.T) {
	mux4 := hl.MuxN(4)
	mux44, err := hw.Chip("myMux4Way4", "a[4], b[4], c[4], d[4], sel[2]", "out[4]", hw.Parts{
...
func TestDMuxNWay(t *testing.T) {
	dmux4, err := hw.Chip("myDMux4Way", "in, sel[2]", "a, b, c, d", hw.Parts{
...
func TestDMuxMWayN(t *testing.T) {
	dmux4 := hl.DMuxN(4)
	dmux44, err := hw.Chip("myDMux4Way4", "in[4], sel[2]", "a[4], b[4], c[4], d[4]", hw.Parts{
